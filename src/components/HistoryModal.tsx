...
import { Download, ArrowUpRight, ArrowDownLeft, ChevronRight, ChevronDown, FolderIcon, FileText } from 'lucide-react';
...
export default function HistoryModal({ open, onClose, senderEmail, senderName }: HistoryModalProps) {
  ...
  const getFileTreeFromPaths = (paths: string[]) => {
  ...
  const HistoryTreeItem = ({ item, level = 0 }: { item: any, level?: number }) => {
    ...
    return (
      <div className="select-none">
        <div 
          className="flex items-center gap-1.5 py-0.5 px-2 hover:bg-muted/50 rounded cursor-pointer transition-colors"
        ...
        >
          ...
          {item.type === 'folder' ? (
            <FolderIcon className="h-3 w-3 text-primary/70" />
          ) : (
        ...
        {hasChildren && isExpanded && (
          <div>
            {Object.values(item.children).map((child: any, idx) => (
              <HistoryTreeItem key={idx} item={child} level={level + 1} />
            ))}
          </div>
        )}
      </div>
                            ...
                            <tr key={r.id} className="border-t border-border/50 hover:bg-muted/30 transition-colors">
                              ...
                              <td className="px-3 py-2">
                                <div className="flex flex-col gap-1">
                                  <span className="truncate max-w-[200px] font-mono text-[11px] font-semibold">{r.file_name}</span>
                                  {r.file_list && r.file_list.length > 0 && (
                                    <div className="mt-1 border-l-2 border-primary/20 ml-1">
                                      <HistoryTreeItem item={getFileTreeFromPaths(r.file_list)} />
                                    </div>
                                  )}
                                </div>
                              </td>
