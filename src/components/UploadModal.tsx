...
export default function UploadModal({ open, mode, onClose, onFileSelected, onFolderSelected }: UploadModalProps) {
  ...
  const handleFolderInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onFolderSelected(files as any, (files[0] as any).webkitRelativePath?.split('/')[0] || 'folder');
    onClose();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md transition-all duration-500 ease-out">
        <DialogHeader>
